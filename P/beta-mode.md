...
* If $\alpha \geq 1$ and $\beta \geq 1$ (but not $\alpha = 1$ and $\beta = 1$), then $0 \leq x = \frac{\alpha-1}{\alpha+\beta-2} \leq 1$ and $f'_X(x) = 0$ and $f'\'\_X(x) < 0$, such that $f_X(x)$ reaches its machimum at $\frac{\alpha-1}{\alpha+\beta-2} = \mathrm{mode}(X)$.