...
2) Using the [law of marginal probability](/D/prob-marg), we have:

$$ \label{eq:ng-marg-x-s1}
\begin{split}
   p(x)
&= \int p(x,y) \, \mathrm{d}y \\
...
Using the [probability density function of the gamma distribution](/P/gam-pdf) and [the fact that probability density functions integrate to one](/D/pdf), we get:

$$ \label{eq:ng-marg-x-s2}
\begin{split}
   p(x)
